################################################################################
# Updated WandBLogger for Avalanche (compatible with wandb >= 0.16)            #
# Author: Updated by ChatGPT (2025)                                            #
################################################################################
...
class WandBLogger(BaseLogger, SupervisedPlugin):
    """Updated Weights & Biases logger for Avalanche."""
    ...
    def log_single_metric(self, name, value, x_plot):
        ...
        # Handle alternative value formats
        ...
        if isinstance(value, Image):
            ...
            value = np.histogram(value.detach().cpu().view(-1).numpy())
            self.wandb.log(
    ...
    def _log_checkpoint(self, name, value, x_plot):
        ...
        checkpoint_directory = Path.cwd() / self.path
        ...
        artifact = self.wandb.Artifact(
            name=checkpoint_name, type="model", metadata=metadata
        )
